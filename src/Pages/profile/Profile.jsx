...
function Profile() {
  ...
  function followUser() {
    console.log("ye h loggg", loggedIn);
    if (loggedIn) {
      if (!follow) {
        console.log("login haiii mc");
        axios
          .post("/api/follow", { user: id })
          .then((res) => console.log(res.data));
        setFollow(true);
      } else {
        console.log("Unfollow mt krr");
        axios
        ...
        <div className="profileSec col-12 d-flex align-items-center pb-5 mt-4">
          ...
          <div className="position-relative" >
            <img
              ...
              alt=""
            />
            {myProfile ? <AddCircleIcon onClick={uploadStory} className="z-3 position-absolute end-0 bottom-0 text-primary" /> : null}
            </div>
          </div>
