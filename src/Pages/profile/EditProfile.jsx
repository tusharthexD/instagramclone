...
function EditProfile() {
  ...
  useEffect(() => {
    axios
      .get("https://instaclonebe-rfqu.onrender.com/api/profile/edit", {
        headers: {
      ...
      try {
        ...
        await uploadBytes(imageRef, compressedFile).then((e) =>
          getDownloadURL(imageRef).then(async (e) => {
            ...
            axios
              .post(
                "https://instaclonebe-rfqu.onrender.com/api/profile/edit",
                user,
                {
                  headers: {
                    Authorization: token ? `Bearer ${token}` : "", // Include Authorization header if token exists
        ...
        <div className="d-flex">
          ...
          <div className="pt-5 flex-grow-1 px-2 px-md-5">
            {photo ? (
              <div>
                <p
                  className="btn z-3 position-absolute bg-primary"
                  onClick={() => croppedImage(profile)}
                >
                  Next
                </p>
                <Cropper
            ...
            <Button
              onClick={uploadImage}
              className="bg-primary fw-bold text-light rounded-2"
            >
              Save
            </Button>
          </div>
